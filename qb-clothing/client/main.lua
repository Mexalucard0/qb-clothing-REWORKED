...
function DisplayHelpText(str)
    ...
    AddTextComponentString("<FONT FACE = 'Arb'>"..(str))
            ...
            for k, v in pairs(Config.Stores) do
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - Para comprar ropa')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - Para cambiar tu corte de pelo')
                            end
                            if IsControlJustPressed(0, Keys["E"]) then
                                ...
                                elseif Config.Stores[k].shopType == "barber" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Cabello", selected = true},
                                    })
                                end
                            end
                        end
                    ...
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                DrawText3Ds(Config.ClothingRooms[k].x, Config.ClothingRooms[k].y, Config.ClothingRooms[k].z + 0.3, '~g~E~w~ - Ver atuendos')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    customCamLocation = Config.ClothingRooms[k].cameraLocation
                                    gender = "Hombre"
                                    if QBCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "Mujer" end
                                    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            ...
                                            {menu = "myOutfits", label = "Mis Atuendos", selected = false, outfits = result},
                                            {menu = "character", label = "Personaje", selected = false},
                                            {menu = "accessoires", label = "Accesorios", selected = false}
                                        })
...
AddEventHandler('qb-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({
            {menu = "myOutfits", label = "Mis atuendos", selected = true, outfits = result},
        })
    end)
end)
...
AddEventHandler('qb-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
        {menu = "character", label = "Personal", selected = true},
        {menu = "clothing", label = "Apariencia", selected = false},
        {menu = "accessoires", label = "Accesorios", selected = false}
    })
end)
...
RegisterNUICallback('TrackerError', function()
    TriggerEvent('chatMessage', "SYSTEM", "error", "No puedes quitarte la tobillera ...")
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    TriggerEvent('chatMessage', "SYSTEM", "warning", "Tu tienes "..data.outfitName.." eliminado!")
end)
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
        TriggerEvent('chatMessage', "SYSTEM", "warning", "Tu tienes "..oData.outfitName.." ¡elegido! Presione Confirmar para confirmar el atuendo.")
    end
end)
